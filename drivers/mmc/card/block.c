	...
	struct mmc_ioc_cmd __user *ic_ptr)
{
	...
	{
		/*cmdq switch off*/
		pr_info("CRLOG ioctl mmc_blk_cmdq_switch off.\n");
		err = mmc_blk_cmdq_switch(card, NULL, false);
		if (err)
		{
			pr_err("%s: %s: ioctl cmdq disable failed %d.\n", mmc_hostname(card->host), __func__, err);
			goto cmd_rel_host;
		}
		cmdq_switch = true;
	}
	...
	{
		/*cmdq switch on*/
		pr_info("ioctl:mmc_blk_cmdq_switch on and mmc_blk_cmdq_reset.\n");
		mmc_blk_cmdq_reset(card -> host, false);
		err = mmc_blk_cmdq_switch(card, NULL, true);
		if (err)
		{
			pr_err("%s: %s: ioctl cmdq enable failed %d.\n", mmc_hostname(card->host), __func__, err);
			goto cmd_rel_host_halt;
		}
	}
...
/*
 * In this kernel, there is no multiple commands ioctl support, so we
 * add the function to make sure the caller do not need to send
 * commands one by one.
 * Without this kind of function, crash may happed if you do not
 * sync the threads correctly when sending the commands with the ioc
 * command function.
 */
				   ...
				   struct mmc_ioc_multi_cmd __user *user)
{
	...
	for (i = 0; i < num_of_cmds && !ioc_err; i++)/*lint !e574*/
		ioc_err = mmc_blk_ioctl_cmd(bdev, &cmds[i]);
	...
	unsigned int cmd, unsigned long arg)
{
	...
	/*
	 * supporting the function to handle the multi cmd ioctl
	 */
...
#ifdef CONFIG_HW_EMMC_PROTECT_MODULE
	ret = mmc_blk_ioctl_huawei_ext(bdev, mode, cmd, arg, ret);
#endif
	return ret;
}
