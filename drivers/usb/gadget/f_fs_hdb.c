...
#define DESC_IDX1       1
#define DESC_IDX0       0
#define EP_DESC_ID2   2
#define EP_DESC_ID1   1
#define EP_DESC_ID0   0
#define MASK_32_BIT    -1
...
static int __hdb_ffs_ep0_queue_wait(struct hdb_ffs_data *hdb_ffs, char *data, size_t len)
{
	 ...
	 */
	if (req->buf == NULL)
		req->buf = (void *)0xDEADBABE;//get a typic num
				     ...
				     size_t n)
{
	...
	if (n < hdb_ffs->ev.count) {
		...
		hdb_ffs->ev.count = 0;//CLEAR COUNT TO 0
	}
		 ...
		 loff_t *ptr)
{
	...
	return hdb_ffs_epfile_io(file, (char __user *)buf, len, FALSE);
}
...
hdb_ffs_epfile_read(struct file *file, char __user *buf, size_t len, loff_t *ptr)
{
	...
	return hdb_ffs_epfile_io(file, buf, len, TRUE);
}
...
static int hdb_ffs_fs_parse_opts(struct hdb_ffs_sb_fill_data *data, char *opts)
{
	...
	for (;;) {
		...
		*eq = FALSE;
		...
		switch (eq - opts) {
		case OPT_5:
			if (!memcmp(opts, "rmode", OPT_5))
				data->root_mode  = (value & OPT_555) | S_IFDIR;
			else if (!memcmp(opts, "fmode", OPT_5))
				data->perms.mode = (value & OPT_666) | S_IFREG;
			else
				goto invalid;
			break;

		case OPT_4:
			if (!memcmp(opts, "mode", OPT_4)) {
				data->root_mode  = (value & OPT_555) | S_IFDIR;
				data->perms.mode = (value & OPT_666) | S_IFREG;
			} else {
				goto invalid;
		...
		case OPT_3:
			if (!memcmp(opts, "uid", OPT_3)) {
				data->perms.uid = make_kuid(current_user_ns(), value);
				if (!uid_valid(data->perms.uid)) {
					pr_err("%s: unmapped value: %lu\n", opts, value);
					return -EINVAL;
				}
			} else if (!memcmp(opts, "gid", OPT_3)) {
	      ...
	      const char *dev_name, void *opts)
{
	...
	struct hdb_ffs_sb_fill_data data = {
		.perms = {
			.mode = S_IFREG | OPT_600,
			.uid = GLOBAL_ROOT_UID,
			.gid = GLOBAL_ROOT_GID,
		},
		.root_mode = S_IFDIR | OPT_500,
	};
	struct dentry *rv = NULL;
	int ret = 0;
	void *hdb_ffs_dev = NULL;
	struct hdb_ffs_data	*hdb_ffs = NULL;
...
static struct hdb_ffs_data *hdb_ffs_data_new(void)
{
	...
	atomic_set(&hdb_ffs->ref, TRUE);
	atomic_set(&hdb_ffs->opened, FALSE);
...
{
	struct usb_gadget_strings **lang = NULL;
   	if((!hdb_ffs)||(!cdev))
           return -ENOMEM;
	ENTER();
...
{
	struct hdb_ffs_epfile *epfile = NULL, *epfiles = NULL;
	...
	for (i = LEN_1; i <= count; ++i, ++epfile) {
		...
		if (!unlikely(hdb_ffs_sb_create_file(hdb_ffs->sb, epfiles->name, epfile,
						 ...
						 &epfile->dentry))) {
			hdb_ffs_epfiles_destroy(epfiles, i - LEN_1);
			return -ENOMEM;
		}
	}
...
static void hdb_ffs_func_eps_disable(struct hdb_ffs_function *func)
{
	...
	spin_lock_irqsave(&func->hdb_ffs->eps_lock, flags);
	do {
		atomic_set(&epfile->error, TRUE);//SET ATOMIC TO 1
				...
				void *priv)
{
	...
	case HDB_FFS_INTERFACE:
		...
		if (*valuep >= hdb_ffs->interfaces_count)
			hdb_ffs->interfaces_count = *valuep + LEN_1;
		break;
				...
				char *const _data, size_t len)
{
    if((!hdb_ffs)||(!_data))
		return -ENOMEM;
	unsigned fs_count = 0 , hs_count = 0, ss_count = 0;
	int fs_len = 0, hs_len = 0, ss_len = 0, ss_magic = 0, ret = -EINVAL;
	char *data = _data;

	ENTER();

	if (unlikely(get_unaligned_le32(data) != FUNCTIONFS_DESCRIPTORS_MAGIC ||
		     get_unaligned_le32(data + LEN_4) != len))
		goto error;
	fs_count = get_unaligned_le32(data +  LEN_8);
	hs_count = get_unaligned_le32(data + LEN_12);
	...
	if ((len >= hs_len + LEN_8)) {
		...
		ss_count = get_unaligned_le32(data + hs_len + LEN_4);
		data += hs_len + LEN_8;
		len  -= hs_len + LEN_8;
	} else {
		data += hs_len;
...
static int __hdb_ffs_data_got_strings(struct hdb_ffs_data *hdb_ffs,
				  char *const _data, size_t len)
{
	u32 str_count = 0, needed_count = 0, lang_count = 0;
	struct usb_gadget_strings **stringtabs = NULL, *t = NULL;
	struct usb_string *strings = NULL, *s = NULL;
	...
	if (unlikely(get_unaligned_le32(data) != FUNCTIONFS_STRINGS_MAGIC ||
		     get_unaligned_le32(data + LEN_4) != len))
		goto error;
	str_count  = get_unaligned_le32(data + LEN_8);
	lang_count = get_unaligned_le32(data + LEN_12);
				    ...
				    void *priv)
{
	...
	struct hdb_ffs_ep *hdb_ffs_ep = NULL;
	...
	if (is_ss) {
		...
		ep_desc_id = EP_DESC_ID2;
	} else if (is_hs) {
		func->function.hs_descriptors[(long)valuep] = desc;
		ep_desc_id = EP_DESC_ID1;
	} else {
		func->function.fs_descriptors[(long)valuep]    = desc;
		ep_desc_id = EP_DESC_ID0;
	}

	if (!desc || desc->bDescriptorType != USB_DT_ENDPOINT)
		return 0;

	idx = (ds->bEndpointAddress & USB_ENDPOINT_NUMBER_MASK) - LEN_1;
	...
	if (hdb_ffs_ep->ep) {
		...
		func->eps_revmap[ds->bEndpointAddress &
				 USB_ENDPOINT_NUMBER_MASK] = idx + LEN_1;
	}
				   ...
				   void *priv)
{
	...
	case HDB_FFS_STRING:
		...
		newValue = func->hdb_ffs->stringtabs[0]->strings[*valuep - LEN_1].id;
		break;
	...
	case HDB_FFS_ENDPOINT:
		...
		idx = (*valuep & USB_ENDPOINT_NUMBER_MASK) - LEN_1;
...

		{
			...
			newValue = descs[descs[0] ? LEN_0 : LEN_1]->bEndpointAddress;
		}
		break;
	}
			 ...
			 struct usb_function *f)
{
	...
	int fs_len = 0, hs_len = 0, ret = 0;

	/* Make it a single chunk, less management later on */
	vla_group(d);
	vla_item(d, struct hdb_ffs_ep, eps, hdb_ffs->eps_count);
	vla_item(d, struct usb_descriptor_header *, fs_descs,
		full ? hdb_ffs->fs_descs_count + 1 : 0);
	vla_item(d, struct usb_descriptor_header *, hs_descs,
		high ? hdb_ffs->hs_descs_count + 1 : 0);
	vla_item(d, struct usb_descriptor_header *, ss_descs,
		super ? hdb_ffs->ss_descs_count + 1 : 0);
	vla_item(d, short, inums, hdb_ffs->interfaces_count);
	vla_item(d, char, raw_descs,
		high ? hdb_ffs->raw_descs_length : hdb_ffs->raw_fs_descs_length);
	...
	memcpy(d_raw_descs, hdb_ffs->raw_descs + 16, d_raw_descs__sz);

	/* Copy SS descriptors */
	if (func->hdb_ffs->ss_descs_count)
		memcpy(d_raw_descs + hdb_ffs->raw_fs_hs_descs_length,
	...
	ret = hdb_ffs_do_descs(hdb_ffs->fs_descs_count +
			   (high ? hdb_ffs->hs_descs_count : LEN_0) +
			   (super ? hdb_ffs->ss_descs_count : LEN_0),
...
static void hdb_ffs_func_disable(struct usb_function *f)
{
    	if(!f)
	    return;
	hdb_ffs_func_set_alt(f, 0, (unsigned)MASK_32_BIT);
}
