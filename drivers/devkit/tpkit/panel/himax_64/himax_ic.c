...
static void himax_touch_information(void)
{
	...
	HX_NC_BT_NUM = data[3];
		...
		//HX_NC_Y_RES				= HX83102_Y_RES;
		//HX_NC_MAX_PT				= HX83102_MAX_PT;
		//HX_NC_XY_REVERSE			= HX83102_XY_REVERSE;
...
static bool himax_ic_package_check(void)
{
	...
	for (i = 0; i < retry; i++)
	{
		    ...
		    (tmp_data[1] == (uint8_t)hx_id_name[2]))
		{
			...
			TS_LOG_INFO("Himax IC package HX83102B \n");
...
static int himax_start_get_rawdata_from_event(int hx_touch_info_size, int RawDataLen, uint8_t *buf)
{
	...
	&& buf[hx_touch_info_size + 2] > 0)//header format check
	{
...
static int himax_irq_bottom_half(struct ts_cmd_node *in_cmd,struct ts_cmd_node *out_cmd)
{
	...
	//TS_LOG_INFO("%s: hx_touch_info_size = %d\n",__func__, hx_touch_info_size);
	 ...
	 if(atomic_read(&g_himax_nc_ts_data->suspend_mode)&&g_himax_nc_ts_data->tskit_himax_data->ts_platform_data->chip_data->easy_wakeup_info.sleep_mode)
	  {
		...
		/*input_report_key(g_himax_nc_ts_data->tskit_himax_data->ts_platform_data->input_dev, info->gesture_wakeup_value , 1);
		input_sync(g_himax_nc_ts_data->tskit_himax_data->ts_platform_data->input_dev);
		input_report_key(g_himax_nc_ts_data->tskit_himax_data->ts_platform_data->input_dev, info->gesture_wakeup_value , 0);
		input_sync(g_himax_nc_ts_data->tskit_himax_data->ts_platform_data->input_dev);*/
		return  retval;
	   }
	   	//TS_LOG_INFO("%s: himax_parse_wake_event end\n",__func__);
	...
	//TS_LOG_INFO("%s: himax_read_event_stack: retval = %d \n",__func__, retval);//use for debug
	if (retval < 0)
#else
...
bypass_checksum_failed_packet:
	...
	//TS_LOG_INFO("%s: himax_parse_coords end \n", __func__);
...
static int himax_parse_specific_dts(struct himax_ts_data *ts, struct himax_i2c_platform_data *pdata)
{
	...
	if (of_property_read_u32_array(dt, "himax,flash-addr", hx_flash_addr,length) == NO_ERR) {
		//ts->hx_flash_addr = hx_flash_addr;
...
static int himax_parse_dts(struct himax_ts_data *ts, struct ts_kit_device_data *chip_data)
{
	...
	if (retval) {
		strncpy(himax_nc_project_id, PRODUCE_ID, HX_PROJECT_ID_LEN);
		TS_LOG_ERR("Not define product id in Dts, use default\n");
	}
	else{
		strncpy(himax_nc_project_id, projectid, HX_PROJECT_ID_LEN);
	}
...
//debug log print 
#endif
	if((flash_tmp_buffer[1] != 0x00) && (flash_tmp_buffer[2] != 0x00) && (flash_tmp_buffer[3] != 0x00) && (flash_tmp_buffer[4] != 0x00)
		&&(flash_tmp_buffer[1] != 0xFF) && (flash_tmp_buffer[2] != 0xFF) && (flash_tmp_buffer[3] != 0xFF) && (flash_tmp_buffer[4] != 0xFF))
		strncpy(himax_nc_project_id, flash_tmp_buffer+1, HX_PROJECT_ID_LEN); /*get project id from flash*/
...
void himax_nc_get_DSRAM_data(uint8_t *info_data)
{
	...
	uint8_t  temp_info_data[(MUTUL_NUM + SELF_NUM) *2 + 4 + 8] = {0}; //max mkey size = 8
	...
	for (i = 0; i < (total_read_times); i++)
	{
		...
		{
			himax_nc_register_read(tmp_addr, max_i2c_size, &temp_info_data[i*max_i2c_size]);
			total_size_temp = total_size_temp - max_i2c_size;
		}
		else
		{	
			//TS_LOG_INFO("last total_size_temp=%d\n",total_size_temp);
			himax_nc_register_read(tmp_addr, total_size_temp % max_i2c_size, &temp_info_data[i*max_i2c_size]);
		}
	...
	/* 4. FW stop outputing */
	//TS_LOG_INFO("DSRAM_Flag=%d\n",DSRAM_Flag);
	if(DSRAM_Flag == false)
	{
		//TS_LOG_INFO("Return to Event Stack!\n");
		himax_rw_reg_reformat_com(ADDR_DSRAM_START,DATA_INIT,tmp_addr,tmp_data);
		himax_flash_write_burst(tmp_addr, tmp_data);
	}
	else
	{
		//TS_LOG_INFO("Continue to SRAM!\n");
...
void himax_nc_reload_disable(void)
{
	uint8_t tmp_addr[4] = {0};
	uint8_t tmp_data[4] = {0};	
...
int himax_nc_switch_mode(int mode)
{
	...
	{
		himax_rw_reg_reformat_com(ADDR_MODE_SWITCH,DATA_NOR_MODE,tmp_addr,tmp_data);	
		mode_wirte_cmd = tmp_data[1];
		mode_read_cmd = tmp_data[0];
	}
	else	/*sorting mode*/
	{
		himax_rw_reg_reformat_com(ADDR_MODE_SWITCH,DATA_SORT_MODE,tmp_addr,tmp_data);	
		mode_wirte_cmd = tmp_data[1];
		mode_read_cmd = tmp_data[0];
	}
	himax_flash_write_burst(tmp_addr, tmp_data);
	...
	if(mode)
	{
		himax_rw_reg_reformat_com(ADDR_MODE_SWITCH,DATA_SORT_MODE_NFRAME,tmp_addr,tmp_data);
		himax_flash_write_burst(tmp_addr, tmp_data);
		himax_nc_reload_disable();
	}
	else
	{		
	...
	while(retry!=0)
	{
		...
		himax_rw_reg_reformat(ADDR_MODE_SWITCH,tmp_addr);
		himax_nc_register_read(tmp_addr, FOUR_BYTE_CMD, tmp_data);
		msleep(100);
