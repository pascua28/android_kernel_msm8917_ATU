...
int himax_nc_chip_self_test(void)
{
	...
	himax_rw_reg_reformat_com(ADDR_HAND_SHAKING,DATA_HAND_SHAKING,tmp_addr,tmp_data);
	...
	for(i = 0;i < 10;i++)
	{
		...
		himax_rw_reg_reformat(ADDR_HAND_SHAKING,tmp_addr);
	...
	if (test_result_id==0xAA) {
		...
		/*TS_LOG_ERR("[Himax]: bank_avg = %d, bank_max = %d,%d,%d, bank_min = %d,%d,%d, key = %d,%d,%d\n",
...
int himax_Raw_Data_test(int step) //for Rawdara
{
	...
	TS_LOG_DEBUG("Raw Data UP Criteria=%d, Down Criteria=%d\n", cap_rawdata_limit[DOWN_LIMIT], cap_rawdata_limit[UP_LIMIT]);	
	for(index1=0;index1<rx *tx;index1++)
	{
		...
		{
			TS_LOG_DEBUG("[PANEL_ISSUE] POS_RX:%3d, POS_TX:%3d, CAP_RAWDATA: %6d\n",(index1 % rx), (index1 / rx), mutual_bank[index1]);
			result = HX_ERR;
		}			
	}
...
static int himax_self_delta_test(int step)
{
	...
	TS_LOG_DEBUG("TX_DELTA UP Criteria=%d, Down Criteria=%d\n", trx_delta_limit[DOWN_LIMIT], trx_delta_limit[UP_LIMIT]);	
	for(index1 = 0;index1 < tx_delta_num; index1++)
	{
		...
		{
			TS_LOG_DEBUG("[PANEL_ISSUE] POS_RX:%3d, POS_TX:%3d, TX_DELTA: %6d\n",(index1 % rx), (index1 / rx), tx_delta[index1]);
			result=HX_ERR;
		}
	...
	TS_LOG_DEBUG("RX_DELTA UP Criteria=%d, Down Criteria=%d\n", trx_delta_limit[DOWN_LIMIT], trx_delta_limit[UP_LIMIT]);		
	/*lint -save -e* */
	for(index1 = 1; index2 < rx_delta_num; index1++)
	{
		...
		{
			TS_LOG_DEBUG("[PANEL_ISSUE] POS_RX:%3d, POS_TX:%3d, RX_DELTA: %6d\n",(index1 % (rx - 1)), (index1 / (rx - 1)), rx_delta[index2]);		
			result=HX_ERR;
		}
	...
	printk("TX start\n");
	for (index1 = 0; index1 < tx_delta_num; index1++) {
		printk("%6d", tx_delta[index1]);
		if ((index1 % (rx)) == (rx - 1))
			printk("\n");
	}
	printk("RX start\n");
	for (index1 = 0; index1 < rx_delta_num; index1++) {
		printk("%6d", rx_delta[index1]);
		if (((index1) % (rx-1)) == (rx - 2))
			printk("\n");
	}
	//=====debug log=====
...
static int himax_iir_test(int step) //for Noise Delta
{
	...
	TS_LOG_DEBUG("NOISE UP Criteria=%d, Down Criteria=%d\n", noise_limit[DOWN_LIMIT], noise_limit[UP_LIMIT]);

	for(index1=0; index1 < rx*tx; index1++)
	{
		if(mutual_iir[index1] > noise_limit[UP_LIMIT])		
		{
			TS_LOG_DEBUG("[PANEL_ISSUE] POS_RX:%3d, POS_TX:%3d, NOISE : %6d\n",(index1 % rx), (index1 / rx), mutual_iir[index1]);		
			result=-1;
		}
...
int himax_nc_factory_start(struct himax_ts_data *ts,struct ts_rawdata_info *info_top)
{
	...
	*/
	...
	/* step4: Open/Short */
	hx_result_status[test4] = himax_basec_test(test5); //for short/open
