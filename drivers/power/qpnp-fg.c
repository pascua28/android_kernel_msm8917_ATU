...
#define FG_FULL_SOC_RAW		255
#define FG_SOC_AT_TERM		100
...
static struct fg_mem_setting settings[FG_MEM_SETTING_MAX] = {
	...
	SETTING(IRQ_VOLT_EMPTY,	 0x458,   3,      3000),//modify the empty irq voltage to 3v
...
static int FULL_SOC_RAW = FG_FULL_SOC_RAW;
static int soc_at_term = FG_SOC_AT_TERM;
...
static int get_prop_capacity(struct fg_chip *chip)
{
    ...
    /* this is from qcom, used to report 100% when the soc is 98% */
				       ...
				       union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHG_ITERM:
		val->intval = settings[FG_MEM_CHG_TERM_CURRENT].value;
		break;
	case POWER_SUPPLY_PROP_VOLTAGE_MAX:
		if (is_charger_available(chip)) {
			return chip->batt_psy->get_property(chip->batt_psy,
					POWER_SUPPLY_PROP_VOLTAGE_MAX, val);
		} else {
			val->intval = chip->batt_max_voltage_uv / UV_TO_MV;
		}
		break;
	default:
		return -EINVAL;
				  ...
				  const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_RESET_LEARNED_CC:
		...
		break;
	case POWER_SUPPLY_PROP_VOLTAGE_MAX:
		chip->cc_cv_threshold_mv = val->intval - CC_CV_DELTA_MV;
...
#define ESR_DEFAULT_VALUE	0x4A67411F61C34A67
...
static void fg_set_esr_cfg(struct fg_chip *chip, u64 default_value, bool pulse_cfg)
{
    ...
    /* this is from qcom, please DONOT change it */
	if (pulse_cfg) {
...
wait:
    ...
    /* itech_3000mA is the default battery profile */
...
#ifdef CONFIG_HUAWEI_PMU_DSM
		dsm_post_chg_bms_info(DSM_BMS_BATT_PROFILE_EXCEPTION,
				"battery profile load failed\n");
#endif
	}
}
...
#define MAX_DECREMENT_DECIPERC	50
static int fg_of_init(struct fg_chip *chip)
{
	...
	settings[FG_MEM_HARD_HOT].value = CONFIG_HLTHERM_TEMP;
	pr_info("set high temp is %d\n",settings[FG_MEM_HARD_HOT].value);
#endif
	...
	OF_READ_PROPERTY(chip->learning_data.max_increment,
			"cl-max-increment-deciperc", rc, 1000);
	OF_READ_PROPERTY(chip->learning_data.max_decrement,
			"cl-max-decrement-deciperc", rc, MAX_DECREMENT_DECIPERC);
