...
#define PA224_DEBUG_MASK           1
#define PA224_ERR_LOG              0
#define PA224_INFO_LOG             1
#define PA224_FLOW_LOG             2

#define PA224_FAR_PS_MIN           255
#define PA224_DELAY_DEFAULT_TIME   100
#define PA224_INVAL_VALUE          (-1)
#define PA224_VALUE_1              1
#define PA224_VALUE_2              2
#define PA224_VALUE_3              3
#define PA224_VALUE_4              4
#define PA224_VALUE_5              5
#define PA224_VALUE_6              6
#define PA224_VALUE_10             10
#define PA224_VALUE_15             15
#define PA224_VALUE_16             16
#define PA224_VALUE_20             20
#define PA224_VALUE_30             30
#define PA224_VALUE_32             32
#define PA224_VALUE_50             50
#define PA224_VALUE_100            100
#define PA224_VALUE_120            120
#define PA224_VALUE_200            200
#define PA224_VALUE_255            255
#define PA224_VALUE_512            512
#define PA224_VALUE_1000           1000
#define PA224_VALUE_0X02           (0x02)
#define PA224_VALUE_0X08           (0x08)
#define PA224_VALUE_0X0C           (0x0C)
#define PA224_VALUE_0X40           (0x40)
#define PA224_VALUE_0X82           (0x82)
#define PA224_VALUE_0XFF           (0xff)

#define PA224_FAR_INIT_VAL         199
#define PA224_NEAR_INIT_VAL        200
...
static unsigned int far_ps_min = PA224_FAR_PS_MIN;
...
static struct sensors_classdev sensors_proximity_cdev = {
	...
	.enabled = 0,
	.delay_msec = PA224_DELAY_DEFAULT_TIME ,
	.sensors_enable = NULL,
...
static int far_init = PA224_FAR_INIT_VAL ;
static int near_init = PA224_NEAR_INIT_VAL;
static int txc_power_delay_flag = PA224_VALUE_1;     /*1 not always on ,0 always on*/
/*
...
static int txc_pa2240_dsm_report_wrong_irq(struct txc_pa2240_data *data)
{
	...
	if ((NULL == data) || (NULL == data->platform_data))
	{
...
static void txc_pa2240_dsm_no_irq_check(struct txc_pa2240_data *data)
{
	...
	*	irq gpio status
	*/
	atomic_set(&excep->ps_enable_flag, PA224_VALUE_1);
	/*delete it, no use check_type*/

	schedule_delayed_work(&data->dsm_irq_work, msecs_to_jiffies(PA224_VALUE_120));
}
...
static void txc_pa2240_dsm_no_update_threhold_check(struct txc_pa2240_data *data)
{
	...
	schedule_delayed_work(&data->dsm_work, msecs_to_jiffies(PA224_VALUE_10));
}
...
static void txc_pa2240_dsm_change_ps_enable_status(struct txc_pa2240_data *data)
{
	...
	*/
	if (atomic_cmpxchg(&data->ls_test_exception.ps_enable_flag, PA224_VALUE_1, 0)){
		data->ls_test_exception.ps_report_flag = PA224_VALUE_1;
	}
}
...
static int txc_pa2240_dump_i2c_exception_status(struct txc_pa2240_data *data)
{
	...
	excep->vdd_mv = regulator_get_voltage(data->vdd)/PA224_VALUE_1000;
	if(excep->vdd_mv < 0){
		TXC_PA2240_ERR("%s,line %d:regulator_get_voltage vdd failed\n",__func__,__LINE__);
	}

	excep->vio_mv = regulator_get_voltage(data->vio)/PA224_VALUE_1000;
	if(excep->vio_mv < 0){
...
static int txc_pa2240_dsm_init(struct txc_pa2240_data *data)
{
	...
	data->ls_test_exception.reg_buf = kzalloc(PA224_VALUE_512, GFP_KERNEL);
						...
						const char *buf, size_t count)
{
	...
	if (strict_strtol(buf, PA224_VALUE_10, &mode))
			return -EINVAL;
...
static ssize_t txc_pa2240_write_file(char *filename, char* param)
{
	...
	if((NULL == filename) || (NULL == param))
	{
		TXC_PA2240_ERR("%s unsupport err_no = %ld: filename or param is NULL!\n", __func__, mode);
		return PA224_INVAL_VALUE;
	}
...
static void txc_pa2240_work_handler(struct work_struct *work)
{
	...
	reg_cfg2 = txc_pa2240_i2c_read(client, TXC_PA2240_REG_CFG2,TXC_PA2240_I2C_BYTE);
	if(pdata == (TXC_PA2240_PROX_MAX_ADC_VALUE + PA224_VALUE_1))
	{
	...
	if (data->irq)
	{
		operate_irq(data,PA224_VALUE_1,true);
	}
#ifdef CONFIG_SENSOR_DEVELOP_TEST
	if(sensorDT_mode)
	{
...
static int pa224_run_calibration(struct i2c_client *client)
{
	...
	u8 cfg0data = 0;
	u8 cfg2data = 0;
	int temp_pdata[PA224_VALUE_20] = {0,};
	unsigned int ArySize = PA224_VALUE_20;
	unsigned int cal_check_flag = 0;
	char buftemp[PA224_VALUE_20] = "\0";
...
RECALIBRATION:
	sum_of_pdata = 0;
	/* Prevent interrput */
	ret = txc_pa2240_i2c_write(client,TXC_PA2240_REG_PS_TH,PA224_VALUE_0XFF,TXC_PA2240_I2C_BYTE);
	if(ret < 0)
	{
	...
	ret = txc_pa2240_i2c_write(client,TXC_PA2240_REG_CFG2,PA224_VALUE_0X08,TXC_PA2240_I2C_BYTE);
	if(ret < 0)
	{
		pdata->flag = PA2240_CALIBRATION_I2C_ERROR;
		TXC_PA2240_ERR("%s,line %d\n",__func__,__LINE__);
	...
	ret = txc_pa2240_i2c_write(client,TXC_PA2240_REG_CFG0,cfg0data | PA224_VALUE_0X02,TXC_PA2240_I2C_BYTE);
	if(ret < 0)
	{
		pdata->flag = PA2240_CALIBRATION_I2C_ERROR;
		TXC_PA2240_ERR("%s,line %d\n",__func__,__LINE__);
...
static int txc_pa2240_run_fast_calibration(struct i2c_client *client)
{
	...
	int sum_of_pdata = 0;
	int xtalk_temp = 0;
    int temp_pdata[PA224_VALUE_4] = {0,};
   	unsigned int ArySize = PA224_VALUE_4;
	u8 cfg0data = 0;
	u8 cfg2data = 0;
	...
	/*PS On*/
	ret += txc_pa2240_i2c_write(client, TXC_PA2240_REG_CFG0,PA224_VALUE_0X02, TXC_PA2240_I2C_BYTE); 
	if(ret < 0)
	{
	...
	/* calculate the cross-talk using central 10 data */
	for (i = PA224_VALUE_1; i < PA224_VALUE_3; i++) 
	{
		TXC_PA2240_FLOW("%s: temp_pdata = %d\n", __func__, temp_pdata[i]);
		sum_of_pdata = sum_of_pdata + temp_pdata[i];
	}

	xtalk_temp = sum_of_pdata/PA224_VALUE_2 ;
	...
	if ((xtalk_temp < pdata->crosstalk + PA224_VALUE_30) && (xtalk_temp > pdata->crosstalk))
	{
		/*calibration success*/
	...
	ret += txc_pa2240_i2c_write(client,TXC_PA2240_REG_CFG2,cfg2data | PA224_VALUE_0X40,TXC_PA2240_I2C_BYTE);
	ret += txc_pa2240_i2c_write(client,TXC_PA2240_REG_CFG0,cfg0data,TXC_PA2240_I2C_BYTE);
	if(ret < 0)
	{
...
static int txc_pa2240_open_ps_sensor(struct txc_pa2240_data *data, struct i2c_client *client)
{
	...
	if ((NULL == data) || (NULL == data->platform_data) || (NULL == client))
	{
	...
	if (data->enable_ps_sensor == 0) {
		...
		*/
		data->enable_ps_sensor= PA224_VALUE_1;
		data->saturation_flag = false;
		data->oil_occur = false;
...
//		txc_pa2240_set_flct(client,(TXC_PA2240_FILTER | (PA2240_PS_FLCT<<4)));
		...
		far_ps_min = PA224_VALUE_255 - oil_effect;
		...
		if (data->irq)
		{
			...
			irq_set_irq_wake(data->irq, PA224_VALUE_1);
		}
	}
	return ret;
}
...
static int txc_pa2240_enable_ps_sensor(struct i2c_client *client,unsigned int val)
{
	...
	if ((val != 0) && (val != PA224_VALUE_1)) {
		TXC_PA2240_ERR("%s: invalid value=%d\n", __func__, val);
		return -EINVAL;
	}
	if (val == PA224_VALUE_1) {
		mutex_lock(&data->single_lock);
		...
		power_key_ps = false;
		schedule_delayed_work(&data->powerkey_work, msecs_to_jiffies(PA224_VALUE_100));
	} else {

		if (data->enable_ps_sensor == PA224_VALUE_1) {
		...
		unsigned int enable)
{
	...
	if ((enable != 0) && (enable != PA224_VALUE_1)) {
		TXC_PA2240_ERR("%s: invalid value(%d)\n", __func__, enable);
		return -EINVAL;
	}
			...
			struct device_attribute *attr, char *buf)
{
	...
	return snprintf(buf,PA224_VALUE_32, "%d\n", pdata);
}
...
{
	struct i2c_client *client = NULL;
	int val_len_max = PA224_VALUE_4;
	char* input_str = NULL;
	char reg_addr_str[PA224_VALUE_10] = {'\0'};
	char reg_val_str[PA224_VALUE_10] = {'\0'};
	long reg_addr = 0;
	long reg_val = 0;
	int addr_lenth = 0;
	int value_lenth = 0;
	...
	snprintf(input_str, PA224_VALUE_10,"%s", buf);
	...
	ret = kstrtol(reg_addr_str,PA224_VALUE_16,&reg_addr);
	if(ret)
		goto parse_fail_exit;

	ret = kstrtol(reg_val_str,PA224_VALUE_16,&reg_val);
	if(ret)
		goto parse_fail_exit;
			...
			struct device_attribute *attr, char *buf)
{
	...
	return snprintf(buf,PA224_VALUE_512,"reg[0x0~0x8]=0x%2x, 0x%2x, 0x%2x, 0x%2x, 0x%2x, 0x%2x, 0x%2x, 0x%2x, 0x%2x\n"
...
static int txc_pa2240_init_client(struct i2c_client *client)
{
	...
	err = txc_pa2240_set_cfg1(client, (pdata->ir_current << PA224_VALUE_4) | ( PA2240_PS_PRST << PA224_VALUE_2));
	if (err < 0)
	{
	...
	/* no long wait */
	err = txc_pa2240_set_cfg3(client, (PA2240_INT_TYPE << PA224_VALUE_6) | (PA2240_PS_PERIOD << PA224_VALUE_3));
	if (err < 0)
	{
		TXC_PA2240_ERR("%s,line%d:txc_pa2240_set_cfg3 FAIL ",__func__,__LINE__);
		return err;
	}
	err = txc_pa2240_set_ps(client,PA224_VALUE_0X82);
	if (err < 0)
	{
		TXC_PA2240_ERR("%s,line%d:txc_pa2240_set_ps FAIL ",__func__,__LINE__);
		return err;
	}
	err = txc_pa2240_set_flct(client,(PA224_VALUE_0X0C | (PA2240_PS_FLCT<<PA224_VALUE_4)));
	if (err < 0)
	{
	...
	err = txc_pa2240_set_piht(client, PA224_VALUE_0XFF);
	if (err < 0)
	{
	...
	err = txc_pa2240_set_cfg2(client, (PA2240_PS_MODE << PA224_VALUE_6) | (PA224_VALUE_1 << PA224_VALUE_2));
	if (err < 0)
	{
...
static void txc_pa2240_parameter_init(struct txc_pa2240_data *data)
{
	...
	data->count = PA224_VALUE_1;	// disable_irq is before enable_irq, so the initial value should more than zero
}
...
static int txc_pa2240_input_init(struct txc_pa2240_data *data)
{
	...
	input_set_abs_params(data->input_dev_ps, ABS_DISTANCE, 0, PA224_VALUE_5, 0, 0);
...
enable_delay:
	if(PA224_VALUE_1 == txc_power_delay_flag)
	{
		msleep(PA224_VALUE_10);
	}
		...
		struct txc_pa2240_platform_data *pdata)
{
	...
	if(rc) {
		...
		txc_power_delay_flag = PA224_VALUE_1;
	}
	else{
...
#ifdef CONFIG_HUAWEI_DEV_DETECT
	err = set_hw_dev_detect_result(DEV_DETECT_APS);
	if (!err)
	{
		TXC_PA2240_ERR("%s %d: set_hw_dev_detect_result failed\n", __func__, __LINE__);
	}
#endif
	/*
