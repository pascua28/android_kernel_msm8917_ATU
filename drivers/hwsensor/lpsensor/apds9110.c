...
#define HW_APDS_ERR_LOG           0
#define HW_APDS_INFO_LOG          1
#define HW_APDS_FLOW_LOG          2

#define HW_APDS_FUN_RET_FAIL      (-1)
#define HW_PS_REPOTED_DATA_NUM    100
#define HW_MDELAY_TIME_2          2
#define HW_MDELAY_TIME_10         10
#define HW_MDELAY_TIME_500        500
#define HW_PS_DATA_AVG_THRE       40
#define HW_PS_PDATA_MASK          (0x7FF)
#define HW_PS_MEAS_VALE_MASK      (0x40)
#define HW_PS_GAIN_INDEX          31
#define HW_PS_DISABLE             0
#define HW_PS_ENABLE              1
#define HW_PS_IRQ_ENABLE          1
#define HW_PS_IRQ_DISABLE         0
#define HW_PS_VAL_ONE             1
#define HW_PS_VAL_ZERO            0
#define HW_PS_PDATA_LEN           32
#define HW_PS_BUF_MAX_LEN         512
#define HW_ABS_PARAM_5            5
#define HW_PS_ID_0XB1             (0xb1)

/*dynamic debug mask to control log print,you can echo value to apds9110_debug to control*/
static int apds9110_debug_mask= HW_APDS_INFO_LOG;
module_param_named(apds9110_debug, apds9110_debug_mask, int, S_IRUGO | S_IWUSR | S_IWGRP);
#define APDS9110_ERR(x...) do {\
	if (apds9110_debug_mask >= HW_APDS_ERR_LOG) \
        printk(KERN_ERR x);\
    } while (0)
/*KERNEL_HWFLOW is for radar using to control all the log of devices*/
#define APDS9110_INFO(x...) do {\
	if (apds9110_debug_mask > HW_APDS_INFO_LOG) \
        printk(KERN_ERR x);\
    } while (0)
#define APDS9110_FLOW(x...) do {\
    if ( apds9110_debug_mask > HW_APDS_FLOW_LOG) \
        printk(KERN_ERR x);\
    } while (0)
...
static struct sensors_classdev sensors_proximity_cdev = {
	...
	.enabled = 0,
	.delay_msec = APDS_DELAY_DEFAULT_VAL,
	.sensors_enable = NULL,
...
static void apds9110_work_handler(struct work_struct *work)
{
	...
	if(pdata < 0){
		...
		data->ps_data = HW_PS_REPOTED_DATA_NUM;
		APDS9110_ERR("%s:i2c error,ps_data = 100",__func__);
	}else{
		data->ps_data = pdata & HW_PS_PDATA_MASK ;
	}
...
static int apds9110_init_client(struct i2c_client *client)
{
	...
	err = apds9110_i2c_write(client,APDS9110_DD_PRX_MEAS_RATE_ADDR, HW_PS_MEAS_VALE_MASK|APDS9110_DD_PRX_DEFAULT_RES|APDS9110_DD_PRX_MEAS_RATE_6_25_MS,APDS9110_I2C_BYTE);
	if (err < 0)
	{
...
static int apds9110_open_ps_sensor(struct apds9110_data *data, struct i2c_client *client)
{
	...
	/* turn on p sensor */
	if (data->enable_ps_sensor == HW_PS_DISABLE) {
		...
		data->enable_ps_sensor = HW_PS_ENABLE;
#ifdef CONFIG_HUAWEI_DSM
		...
		input_sync(data->input_dev_ps);
		if (data->irq){
			operate_irq(data,HW_PS_IRQ_ENABLE,true);
			/*set the property of pls irq,so the pls irq can wake up the sleeping system */
			irq_set_irq_wake(data->irq, HW_PS_IRQ_ENABLE);
		}
	}
	return ret;
}
...
static int apds9110_ps_calibration(struct i2c_client *client)
{
	...
	apds9110_dd_set_prx_meas_rate(client, HW_PS_MEAS_VALE_MASK|APDS9110_DD_PRX_MEAS_RES_8_BIT|APDS9110_DD_PRX_MEAS_RATE_6_25_MS);			
	apds9110_dd_set_main_ctrl(client, APDS9110_DD_PRX_EN);
	data->ps_gain_index = HW_PS_GAIN_INDEX;
	while (data->ps_gain_index > 0)
	{
		...
		for (i=0; i<APDS9110_PS_CAL_LOOP; i++) {
			mdelay(HW_MDELAY_TIME_10);	// must be greater than prx meas rate
...
static int apds9110_enable_ps_sensor(struct i2c_client *client,unsigned int val)
{
	...
	if ((val != HW_PS_VAL_ZERO) && (val != HW_PS_VAL_ONE)) {
		APDS9110_ERR("%s: invalid value=%d\n", __func__, val);
		return -EINVAL;
	}
	if (val == HW_PS_VAL_ONE) {
		mutex_lock(&data->single_lock);
		...
		if (data->enable_ps_sensor==HW_PS_ENABLE) {

			mutex_lock(&data->single_lock);
			
			data->enable_ps_sensor = HW_PS_DISABLE;
			data->enable= 0x00;
...
//			apds9110_i2c_write(client,APDS9110_DD_INT_CFG_ADDR,0x00,APDS9110_I2C_BYTE);
			...
			if (data->irq)
			{
				...
				irq_set_irq_wake(data->irq, HW_PS_IRQ_DISABLE);
				operate_irq(data,HW_PS_IRQ_DISABLE,true);
			}
		}
	}

	/* Vote off  regulators if both light and prox sensor are off */
	if ((data->enable_ps_sensor == HW_PS_DISABLE) &&(data->platform_data->power_on)){
		...
		unsigned int enable)
{
	...
	if ((enable != HW_PS_DISABLE) && (enable != HW_PS_ENABLE)) {
		APDS9110_ERR("%s: invalid value(%d)\n", __func__, enable);
		return -EINVAL;
	}
			...
			struct device_attribute *attr, char *buf)
{
	...
	if(pdata < 0){
		...
		pdata &= HW_PS_PDATA_MASK;
	}

	return snprintf(buf, HW_PS_PDATA_LEN, "%d\n", pdata);
}
			...
			struct device_attribute *attr, char *buf)
{
	...
	for(i = 0,j = 0; i < APDS9110_REG_MAX; j++ )
	{
		...
		if( APDS_VALUE_0X03== i){
			i += APDS_VALUE_0X03;
		}else if(APDS_VALUE_0X09==i){
			i += APDS_VALUE_0X10;
		}else{
			i += APDS_VALUE_1;
		}
	...
	return snprintf(buf,HW_PS_BUF_MAX_LEN,"reg[0x0~0x3]=0x%2x, 0x%2x, 0x%2x, 0x%2x\n"
			...
			struct device_attribute *attr, char *buf)
{
	...
	if(NULL==data)
	{
		return snprintf(buf, HW_PS_PDATA_LEN , "pdata is null\n");
	}
	return snprintf(buf, HW_PS_PDATA_LEN,"%d\n", data->cal_flag);
}
...
static void apds9110_parameter_init(struct apds9110_data *data)
{
	...
	data->count = APDS_VALUE_1;	// disable_irq is before enable_irq, so the initial value should more than zero
	data->ps_gain_index = HW_PS_GAIN_INDEX;      //default gain
}
...
static int apds9110_input_init(struct apds9110_data *data)
{
	...
	input_set_abs_params(data->input_dev_ps, ABS_DISTANCE, 0, HW_ABS_PARAM_5, 0, 0);
...
static void apds9110_powerkey_screen_handler(struct work_struct *work)
{
	...
	schedule_delayed_work(&data->powerkey_work, msecs_to_jiffies(HW_MDELAY_TIME_500));
}
				   ...
				   const struct i2c_device_id *id)
{
	...
	if (!err)
	{
		APDS9110_ERR("%s: set_hw_dev_detect_result fail, err: %d\n", __func__, err);
	}
#endif
